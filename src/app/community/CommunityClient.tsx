...
export default function CommunityClient({ initialPosts, isDbFallback: serverFallback }: CommunityClientProps) {
  const [posts, setPosts] = useState<Post[]>(initialPosts);
  const [category, setCategory] = useState<CategoryFilter>("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [isWriteModalOpen, setIsWriteModalOpen] = useState(false);
  const [isFallbackMode, setIsFallbackMode] = useState(serverFallback);
  ...
  // Hydrate local storage posts on mount if database fallback was active on server
  useEffect(() => {
    if (serverFallback) {
      setPosts(getPosts());
    } else {
      // Sync client state with potential server update
      setPosts(initialPosts);
    }
  }, [serverFallback, initialPosts]);
